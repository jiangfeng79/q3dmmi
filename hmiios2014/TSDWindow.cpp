...
TSDWindow::TSDWindow()
    : m_program(0),
      ...
      m_sgBusVehicles(BUS_TRACKS, BUS_TRACKS_TEXT, new BusLayerParser(BusLayerParser::Vehicles), m_displayMask, *this,
                      LiveMapLayer::LabelStyle::Bus, MapLayer::FillMode::Fill),
      m_sgBusWindshields(BUS_TRACKS_WINDSHIELD, BUS_STOPS_TEXT, new BusLayerParser(BusLayerParser::VehicleWindshields),
                         m_displayMask, *this, LiveMapLayer::LabelStyle::Bus, MapLayer::FillMode::Fill),
      m_mrt(nullptr),
      m_mrtVBO(0),
      m_eblVBO(0),
      m_shader(0),
...
void TSDWindow::render()
{
    ...
    // Live bus arrival labels (service, ETA, load, type) generated by the
    // vehicle parser and drawn through the normal map-layer text pass.
    m_sgBusVehicles.drawText(layerContext);

    // Bus route labels (direction, description, stop code) generated by the
    // route-stop parser. Only shown when zoomed in, to avoid clutter.
    if (SCALE > 0.5)
    {
        m_sgBusRouteLines.drawText(layerContext);
        m_sgBusRouteLines2.drawText(layerContext);
    }
