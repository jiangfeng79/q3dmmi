...
#define X_SCREEN_COORD_TO_WGS84(X) m_sgCoastal.m_property.centerX-m_fMapCenterDeltaX/(m_sgCoastal.m_property.mapBuildScale*m_sgCoastal.m_property.scale)+((X)-width()/2)/(m_sgCoastal.m_property.mapBuildScale*m_sgCoastal.m_property.scale*m_fScaleFactor)
#define Y_SCREEN_COORD_TO_WGS84(Y) WGS84WEBMERCATOR_TO_WGS84(m_sgCoastal.m_property.centerY+m_fMapCenterDeltaY/(m_sgCoastal.m_property.mapBuildScale*m_sgCoastal.m_property.scale)-((Y)-height()/2)/(m_sgCoastal.m_property.mapBuildScale*m_sgCoastal.m_property.scale*m_fScaleFactor))

// lat-long position to screen position, for drawing text
#define X_WGS84_COORD_TO_SCREEN_COORD(X) width()/2+((X)-m_sgCoastal.m_property.centerX+m_fMapCenterDeltaX/(m_sgCoastal.m_property.mapBuildScale*m_sgCoastal.m_property.scale))*m_sgCoastal.m_property.scale*m_fScaleFactor*m_sgCoastal.m_property.mapBuildScale
#define Y_WGS84_COORD_TO_SCREEN_COORD(Y) height()/2-(WGS84_TO_WGS84WEBMERCATOR(Y)-m_sgCoastal.m_property.centerY-m_fMapCenterDeltaY/(m_sgCoastal.m_property.mapBuildScale*m_sgCoastal.m_property.scale))*m_sgCoastal.m_property.scale*m_fScaleFactor*m_sgCoastal.m_property.mapBuildScale
...
// for msg queue
extern hmiios2014 * G_P_MAINWINDOW;
//extern char msgBuf[];
// end message queue
...
void TSDWindow::drawTextWithAngle(TSDWindow::MapLayer &a_layer)
{
	if(a_layer.m_text_id & m_displayMask)
	{
		...
		{
			int x = X_WGS84_COORD_TO_SCREEN_COORD((a_layer.m_dbfFileReader.getEntity())[i].coordinate[0]);
			int y =  Y_WGS84_COORD_TO_SCREEN_COORD((a_layer.m_dbfFileReader.getEntity())[i].coordinate[1]);
...
void TSDWindow::render()
{
	...
	renderShape(QRect(0,0,300,80));
	renderText(10,18, QString("Coord: [%1,%2]").arg(X).arg(Y));
	renderText(10,36, QString("Scale: [%1]").arg(SCALE));
	renderText(10,54, QString("1 NM:  "));
	QVector<QPointF> scaleLine;
	scaleLine.append(QPointF(84,54));
	scaleLine.append(QPointF(84+1852*SCALE,54));
	drawLines(scaleLine);

	renderText(10,72, QString("Refresh Rate: %1").arg(m_fps, 4, 10, QChar(' ')));
	...
	if(SCALE>0.1)
	{
		...
		for(int i=0;i<sizeof(mrt)/sizeof(GLfloat)/2;++i)
		{
			int x = X_WGS84_COORD_TO_SCREEN_COORD(mrt[i*2]);
			int y =  Y_WGS84_COORD_TO_SCREEN_COORD(mrt[i*2+1]);
...
#define granularity  63
	...
	if(m_uiMapOpMask == EBL)
	{
		...
		if(m_bMouseIsPressing)
		{
			...
			renderText(m_iMousePosX+15,m_iMousePosY+20, QString(tr("Angle: %1, Dist: %2")).arg(angle, 5,'f', 1, QChar('0')).arg(r), QString("Tahoma"));
		}
	}
}
//! [6]
...
void TSDWindow::slot_process_msg()
{
	//for(int i=0;i<10;++i)

	//	qDebug() << msgBuf[0] << " "<< msgBuf[1] << " "<< msgBuf[2] << " "<< msgBuf[3] << " "<< msgBuf[4] << " "<< msgBuf[5] << " "<< msgBuf[6] << " \n";
	//	abc * msg = (abc *)msgBuf;
	//	qDebug() << "id: " <<  msg->a << " length:" << msg->b;
