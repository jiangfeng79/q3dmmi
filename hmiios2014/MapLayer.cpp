...
MapLayer::MapLayer(std::uint64_t id, std::uint64_t textId, LayerParser* parser, const std::uint64_t& displayMask,
                   TSDWindow& window)
    : m_parser(parser), m_id(id), m_textId(textId), m_displayMask(displayMask), m_window(window)
{
...
MapLayer::MapLayer(const char* fileName, std::uint64_t id, std::uint64_t textId, LayerParser* parser,
    ...
    : m_fileName(fileName), m_parser(parser), m_id(id), m_textId(textId), m_displayMask(displayMask), m_window(window)
{
    m_VBO_ID[0] = m_VBO_ID[1] = 0;
    m_property.scale = 0.1f;
}

MapLayer::MapLayer(const char* fileName, const char* layerName, std::uint64_t id, std::uint64_t textId,
                   LayerParser* parser, const std::uint64_t& displayMask, TSDWindow& window)
        : m_fileName(fileName), m_layerName(layerName), m_parser(parser), m_id(id), m_textId(textId),
          m_displayMask(displayMask), m_window(window)
{
    m_VBO_ID[0] = m_VBO_ID[1] = 0;
    m_property.scale = 0.1f;
}
...
void MapLayer::buildLayer(const MapProperty& baseProperty, int layerDepth)
{
    ...
    options.useWgs84BuildTransform = !m_layerName.isEmpty() || m_fileName.endsWith("railways");
...
void MapLayer::uploadGeometry(GLenum usage)
{
    ...
    m_window.glBufferData(GL_ARRAY_BUFFER, m_geometry.vertices.size() * sizeof(Vertex), m_geometry.vertices.data(), usage);
...
void MapLayer::drawPrimitive(const MapLayerRenderContext& context) const
{
    ...
    if (type == SHPT_POLYGON && !m_VBO_ID[1])
    {
...
void BaseMapLayer::drawFilled(const MapLayerRenderContext& context) const
{
    ...
    m_window.glClearStencil(0);
    m_window.glEnable(GL_STENCIL_TEST);
    if (m_fillMode == FillMode::WaterArea)
    {
...
BaseMapLayer::BaseMapLayer(const char* fileName, std::uint64_t id, std::uint64_t textId, LayerParser* parser,
    ...
    : MapLayer(fileName, id, textId, parser, displayMask, window), m_fillMode(fillMode)
{
}

BaseMapLayer::BaseMapLayer(const char* fileName, const char* layerName, std::uint64_t id, std::uint64_t textId,
                           LayerParser* parser, const std::uint64_t& displayMask, TSDWindow& window, FillMode fillMode)
    : MapLayer(fileName, layerName, id, textId, parser, displayMask, window), m_fillMode(fillMode)
{
}
...
void BaseMapLayer::buildLayer(const MapProperty& baseProperty, int layerDepth)
{
    if (!m_parser)
        return;
    if (m_fillMode == FillMode::Coastal)
    {
...
void StaticMapLayer::uploadStaticGeometry()
{
    uploadGeometry(GL_STATIC_DRAW);
}
...
LiveMapLayer::LiveMapLayer(std::uint64_t id, std::uint64_t textId, LayerParser* parser,
    ...
    : MapLayer(id, textId, parser, displayMask, window), m_labelStyle(labelStyle)
{
}
